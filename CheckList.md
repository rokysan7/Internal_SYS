...
- [x] API ↔ React UI 통합 동작 확인
  - [x] CRUD 전체 흐름 (생성 → 조회 → 수정 → 삭제)
  - [x] 댓글 등록 → 알림 생성 → Badge 갱신
  - [x] 메모 작성 → 조회
- [x] 알림 시나리오 테스트
  - [x] 댓글 등록 → 담당자 알림 생성 → Badge 반영
  - [x] 미처리 CS 리마인드 → Notification 생성 → Badge 반영
- [x] Celery Task 정상 동작 확인 (Redis 브로커, 태스크 상태)
- [x] 페이지 간 네비게이션 및 데이터 흐름 확인
