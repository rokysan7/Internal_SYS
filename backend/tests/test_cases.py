...
def test_create_case(client, test_user, sample_product, sample_license):
...
def test_create_case_with_assignee_creates_notification(
    client, test_user, assignee_user, sample_product
):
    resp = client.post("/cases/", json={
        "title": "Assigned Case",
        "content": "Content",
...
def test_create_case_without_assignee_no_notification(
    client, test_user, sample_product
):
...
def test_list_cases_filter_assignee(client, sample_case, assignee_user):
    ...
    assert resp.status_code == 200
    for c in resp.json():
        assert c["assignee_id"] == assignee_user.id
...
def test_update_case_change_assignee_creates_notification(
    ...
    # 담당자 변경
    client.put(f"/cases/{case_id}", json={"assignee_id": assignee_user.id})
...
def test_similar_cases(client, test_user, sample_product):
    client.post("/cases/", json={
        "title": "ChatGPT 결제 오류",
        "content": "결제가 안됩니다",
        "requester": "Cust",
    })
    resp = client.get("/cases/similar", params={"query": "결제"})
    assert resp.status_code == 200
    results = resp.json()
...
def test_similar_cases_no_match(client, test_user):
    resp = client.get("/cases/similar", params={"query": "zzzznonexistent"})
    assert resp.status_code == 200
