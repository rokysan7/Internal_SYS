...
def test_bulk_upload_products_success(client, test_user):
    ...
    assert data["products_existing"] == 1  # ChatGPT appears twice
    assert data["licenses_existing"] == 0
    assert data["errors"] == []


def test_bulk_upload_products_dedup(client, test_user, sample_product, sample_license):
...
def test_bulk_upload_invalid_file_type(client, test_user):
...
def test_bulk_upload_missing_columns(client, test_user):
    """필수 컬럼 누락 시 400 에러."""
...
def test_bulk_upload_empty_values(client, test_user):
